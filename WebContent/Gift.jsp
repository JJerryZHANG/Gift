...
<link rel="stylesheet" href="<%=path%>/css/ddsmoothmenu.css"
	type="text/css" />
<link rel="stylesheet" href="<%=path%>/css/gift.css" type="text/css" />
				...
				<div id="content" class="right">
					...
					<div id="info">
						<div id="gift_info">
							<div id="photo">
								<img
									src="<%=path%>/images/product/<%=session.getAttribute("gift_pic")%>">
							</div>
							<div id="basic">

								<ul>
									<li>礼品名称：<a href="#"><%=session.getAttribute("gift_name")%></a></li>
									<li>购买链接：<a href="<%=session.getAttribute("gift_link")%>"><%=session.getAttribute("gift_link")%></a></li>
									<li><h3>标签：</h3>
										<ul>
											<li>所属节日：<a href=""><%=session.getAttribute("gift_day")%><a></a></li>
											<li>所属年龄：<a href=""><%=session.getAttribute("gift_age")%></a></li>
											<li>所属个性：<a href=""><%=session.getAttribute("gift_tag")%></a></li>
										</ul>

									</li>
									<li>相关文章：<a href="servlet/StoryServlet?id=<%=session.getAttribute("storyid")%>"><%=session.getAttribute("gift_story")%></a></li>
								</ul>

							</div>
						</div>
						<div>
							<iframe src='<%=path%>/txt/gift_txt/<%=session.getAttribute("gift_about")%>'></iframe>
						</div>
						<div height="20px">&nbsp;</div>
						<div id="gift_related">
							相似礼品推荐： <a
